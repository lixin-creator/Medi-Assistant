...
from app.agents.serper import SerperAgent  # noqa: E402
...
def test_planner_agent_medical_chinese():
    state = initialize_conversation_state()
    state["question"] = "发烧有哪些症状"
    new_state = PlannerAgent(state)
    assert new_state["current_tool"] == "retriever"
...
# --- Serper Agent Tests ---
def test_serper_agent():
    state = initialize_conversation_state()
    state["question"] = "News"
    with patch('app.agents.serper.get_serper_search') as mock_get:
        mock_serper = MagicMock()
        mock_serper.invoke.return_value = [
            {"content": "News about medical discoveries is important. " * 5, "url": "http://news.com"}]
        mock_get.return_value = mock_serper
        new_state = SerperAgent(state)
        assert new_state["serper_success"] is True


def test_serper_agent_no_tool():
    state = initialize_conversation_state()
    with patch('app.agents.serper.get_serper_search', return_value=None):
        new_state = SerperAgent(state)
        assert new_state["serper_success"] is False


def test_serper_agent_fail():
    state = initialize_conversation_state()
    with patch('app.agents.serper.get_serper_search') as mock_get:
        mock_serper = MagicMock()
        mock_serper.invoke.side_effect = Exception("error")
        mock_get.return_value = mock_serper
        new_state = SerperAgent(state)
        assert new_state["serper_success"] is False
...
def test_executor_agent_no_llm_with_docs_uses_fallback_summary():
    state = initialize_conversation_state()
    state["question"] = "What is X?"
    state["documents"] = [Document(page_content="X is Y. This sentence comes from retrieved docs.")]

    with patch('app.agents.executor.get_llm', return_value=None):
        new_state = ExecutorAgent(state)

    assert "X is Y" in new_state["generation"]
    assert "temporarily unavailable" not in new_state["generation"]
    assert new_state["source"] == "Local Knowledge Fallback"
