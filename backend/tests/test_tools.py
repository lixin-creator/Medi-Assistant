...
import app.tools.serper_search as serper_module  # noqa: E402
...
from app.tools.serper_search import get_serper_search  # noqa: E402
...
def test_get_serper_no_key():
    serper_module._serper_search = None
    with patch('app.tools.serper_search.SERPER_API_KEY', None):
        result = get_serper_search()
        assert result is None


def test_get_serper_with_key():
    serper_module._serper_search = None
    with patch('app.tools.serper_search.SERPER_API_KEY', 'fake-key'):
        result = get_serper_search()
        assert result is not None
    serper_module._serper_search = None
...
def test_vector_store_get_or_create_sklearn_backend():
    vs_module._vectorstore = None
    vs_module._embeddings = MagicMock()

    with patch.dict('os.environ', {'VECTOR_STORE_BACKEND': 'sklearn'}):
        with patch('app.tools.vector_store.os.path.exists', return_value=False):
            with patch('app.tools.vector_store.os.makedirs'):
                with patch(
                    'langchain_community.vectorstores.sklearn.SKLearnVectorStore.from_documents'
                ) as mock_from_documents:
                    mock_store = MagicMock()
                    mock_from_documents.return_value = mock_store

                    res = get_or_create_vectorstore(
                        documents=[MagicMock()], persist_dir='new_sklearn'
                    )

                    assert res is mock_store
...
def test_vector_store_uses_sklearn_backend_by_default_on_windows():
    with patch('app.tools.vector_store.sys.platform', 'win32'):
        with patch.dict('os.environ', {}, clear=True):
            assert vs_module._get_vectorstore_backend() == 'sklearn'
